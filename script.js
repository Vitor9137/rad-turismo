...
document.addEventListener('DOMContentLoaded', function () {
  ...
  if (quoteForm) {
    quoteForm.addEventListener('submit', function (e) {
      ...
      var url = 'https://wa.me/553131603162?text=' + encodeURIComponent(linhas);
      window.open(url, '_blank');
    });
  }


  /* ── 6. Carrossel de fotos da frota ───────────────────────── */
  document.querySelectorAll('[data-carousel]').forEach(function (carousel) {
    var images = carousel.querySelectorAll('.fleet-carousel-track img');
    var dots   = carousel.querySelectorAll('.fleet-carousel-dots button');
    var prev   = carousel.querySelector('.fleet-carousel-arrow.prev');
    var next   = carousel.querySelector('.fleet-carousel-arrow.next');
    var current = 0;

    if (images.length <= 1) {
      if (prev) prev.style.display = 'none';
      if (next) next.style.display = 'none';
      var dotsWrap = carousel.querySelector('.fleet-carousel-dots');
    ...
    function show(index) {
      ...
      images.forEach(function (img, i) { img.classList.toggle('is-active', i === current); });
